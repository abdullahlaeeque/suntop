    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-50">
            <div className="container mx-auto px-4 py-4">
                ...
                <div className="bg-white/70 backdrop-blur-md rounded-2xl border border-orange-200 shadow-md shadow-orange-100">
                            ...
                            <nav className="hidden lg:flex items-center space-x-8">
                                {navItems.map((item) => (
                                    <Link
                                        key={item.name}
                                        href={item.href}
                                        className="relative text-gray-700 hover:text-orange-600 font-medium transition-colors duration-200 group/nav"
                                    >
                                        {item.name}
                                        <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-orange-500 transition-all duration-300 group-hover/nav:w-full"></span>
                                    </Link>
                                ))}
                            </nav>
                                    ...
                                    <Sheet>
                                        <SheetTrigger asChild>
                                            <Button
                                                variant="ghost"
                                                size="icon"
                                                className="lg:hidden rounded-xl bg-white/80 backdrop-blur-sm border border-orange-200 hover:bg-orange-50 hover:shadow-md transition-all duration-200"
                                            >
                                                <Menu className="h-5 w-5 text-gray-600" />
                                            </Button>
                                        </SheetTrigger>
                                        <SheetContent className="bg-white/90 backdrop-blur-md border-l border-orange-200 ps-3">
                                            <div className="flex flex-col space-y-6 mt-8">
                                                {/* Mobile Logo */}
                                                <div className="flex items-center space-x-3 pb-6 border-b border-orange-200">
                                                ...
                                                {navItems.map((item) => (
                                                    <Link
                                                        key={item.name}
                                                        href={item.href}
                                                        className="text-lg font-medium text-gray-700 hover:text-orange-600 transition-colors py-2 border-b border-orange-100"
                                                    >
                                                        {item.name}
                                                    </Link>
                                                ))}
